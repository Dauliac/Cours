...
This produces a `result` file that is a Docker-compatible image archive.
...
### Exercise 2: Explore the closure

```bash
# See exactly what goes into your container
nix path-info -rsh .#default
```

This shows every dependency, its size, and the total closure size. This is what ends up in your container - nothing more, nothing less.
...
| `nix path-info -rsh .#default` | Show the dependency closure |
